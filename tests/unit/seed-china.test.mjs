/**
 * tests/unit/seed-china.test.mjs — CHINA_SEED fixture validation
 * (CONTRACTS-SEASON §1).
 *
 * Checks:
 ...
 *   - player/team ids unique and globally namespaced under the `cn-` prefix;
 *     every roster id resolves to a real player; every player's contract.teamId
 *     points at a real team and appears in that team's roster.
...
import { CHINA_SEED } from '../../src/data/seed/china.js';
...
export default async function seedChinaTest() {
  section('seed/china — structure');

  const { league, teams, players } = CHINA_SEED;
  assert(league && typeof league === 'object', 'CHINA_SEED.league missing');
  assert(Array.isArray(teams), 'CHINA_SEED.teams must be an array');
  assert(Array.isArray(players), 'CHINA_SEED.players must be an array');

  // league shape
  assertEqual(league.id, 'china', 'league.id should be "china"');
  assertEqual(league.region, 'china', 'league.region should be "china"');
  ...
  // EXACTLY 12 China teams, 60 players.
  ...
  section('seed/china — unique, cn-prefixed ids');

  const teamIds = new Set();
  for (const t of teams) {
    assert(typeof t.id === 'string' && t.id.length > 0, 'team.id missing');
    assert(t.id.startsWith('cn-'), `team id must be cn- prefixed: ${t.id}`);
  ...
  for (const p of players) {
    ...
    assert(p.id.startsWith('cn-'), `player id must be cn- prefixed: ${p.id}`);
    assert(!playerIds.has(p.id), `duplicate player id: ${p.id}`);
    playerIds.add(p.id);
  }

  section('seed/china — rosters (12 x exactly 5)');
  ...
  section('seed/china — domain normalization');
  ...
  for (const t of teams) {
    ...
    assertEqual(nt.leagueId, 'china', `normalized team ${t.id} leagueId should be china`);
  }

  // eslint-disable-next-line no-console
  console.log(
    `seed/china: ${teams.length} teams, ${players.length} players normalized cleanly`
  );
}
